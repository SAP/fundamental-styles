...
selector: fd-card
cssFile: card.css
sourcePath: packages/styles/stories/Components/card/card.stories.js
generatedAt: 2026-05-28T16:01:41.641Z
---
...
| Class | Description |
|-------|-------------|
| `fd-card--analytical` | The analytical card is used for data visualization. It can display a KPI header and various chart types in the content. The only difference between a KPI header and a standard header is that the former requires a subtitle, a KPI area and can display an optional second subtitle. To display an analytical card, add the \ |
| `fd-card--interactive` | Non-interactive cards have <code>role="region"</code>. In this case the card can be placed in any container. This role covers the interactive variants: |
| `fd-card--object` | The object card can display a single object or a group of objects. Contrary to other cards, object card has 1rem padding on all sides of the content area. To display an object card, add the \ |
| `fd-card--table` | Alternatively to an Avatar a Message Icon can be displayed within the Main Header. This will be available in three different sizes (this is only applicable for Message Icons not Avatar component on Card Header). The Message Icons are non-interactive. |
...
Non-interactive cards have <code>role="region"</code>. In this case the card can be placed in any container. This role covers the interactive variants:
- no interactivity
...
### Header Interaction - non-interactive Cards

For interactive header use the modifier class <code>.fd-card__header--interactive</code> on <code>.fd-card__header</code>.
...
### Interactive card with role button

The role <b>button</b> can be assigned to an interactive card that has <b>NO</b> interactive element inside. In this case, the card could be placed in any container.
...
The numeric header block is designed for displaying numeric information.
...
Use Illustrated Messages component inside the Card content to represent an empty state. The content container has 1rem spacing.
...
The analytical card is used for data visualization. It can display a KPI header and various chart types in the content. The only difference between a KPI header and a standard header is that the former requires a subtitle, a KPI area and can display an optional second subtitle. To display an analytical card, add the \
...
A card can display various types of lists. All components placed inside should behave natively. For this kind of card it is not recommended to keep header interactive, or navigable.
...
The object card can display a single object or a group of objects. Contrary to other cards, object card has 1rem padding on all sides of the content area. To display an object card, add the \
...
This documentation was automatically generated from: `packages/styles/stories/Components/card/card.stories.js`
