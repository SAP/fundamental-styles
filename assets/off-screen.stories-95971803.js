...
`;const n={title:"Dev/Off Screen",parameters:{description:"For aria-live to read any change in element, we need to place element but it should not be visible to user.\n            At the same time, it can not be made `visibility: hidden` or `display: none`, as screen reader will not be able to pick this element.\n            This component will make sure that element is not visible on screen but it can be read by screen reader.",tags:["a11y"]}},e=()=>a;e.parameters={docs:{story:{iframeHeight:200},description:{story:`For aria-live to read any change in element, we need to place element but it should not be visible to user.
...
//# sourceMappingURL=off-screen.stories-95971803.js.map
