...
`}},r=()=>K;r.parameters={docs:{story:{iframeHeight:300},description:{story:"The basic structure of a menu. Place the menu item title in a `<span>` tag using class `fd-menu__title` and wrap it in the `fd-menu__link` and `fd-menu__item` classes as shown.\n               <br><b>Note:</b>Use compact menus for the Desktop and use cozy for the Tablet mode."}}};const e=()=>J;e.storyName="Vertical overflow";e.parameters={docs:{story:{iframeHeight:300},description:{story:'The `fd-menu--overflow` modifier class will clip the content and add a vertical scroll to the element. You need to manually set the max-height of the element on the `fd-menu` level. For example: `style="max-height: 120px;"`. <br><b>Important: </b>this modifier class cannot be used in cases where the menu has a submenu.'}}};const m=()=>G;m.parameters={docs:{story:{iframeHeight:300},description:{story:"The basic structure of a menu in mobile where it opens as a dialog. Use class `fd-menu--mobile` on menu container level. Example shows the parent menu's item in active state to simulate a pressed/touched event. Submenu appears in its own fullscreen dialog in mobile devices. The device's back button takes one back to the parent menu fullscreen dialog."}}};const n=()=>q;n.storyName="List with separated items";n.parameters={docs:{story:{iframeHeight:300},description:{story:"Separators can be added between Menu Items by adding the `has-separator` class to the element with class `fd-menu__item`. The separator will appear under this item."}}};const s=()=>Y;s.storyName="List different states";s.parameters={docs:{story:{iframeHeight:300},description:{story:"\nSimulate different states to show state behaviour between the items.\n\n- For simulating hover state, use class `is-hover` on the menu item.\n- For simulating active state, use class `is-active` on the menu item.\n- For simulating selected state, use class `is-selected` on the menu item.\n- For simulating selected-hover state, use classes `is-selected is-hover` on the menu item.\n- For simulating disabled state, use class `is-disabled` on the menu item.\n\nTo remove default box shadow from menu containers use `fd-menu__list--no-shadow` or `fd-menu__sublist--no-shadow`."}}};const a=()=>z;a.storyName="List with Icon";a.parameters={docs:{story:{iframeHeight:220},description:{story:"\nTo create an addon before or after `fd-menu__title` element, use elements with folowing classes inside `fd-menu__link`:\n- `fd-menu__addon-before`   - styles addon befotre `fd-menu__title`\n- `fd-menu__addon-after`    - styles addon after `fd-menu__title`\n- `fd-menu__shortcut`       - styles shortcut placed after `fd-menu__title`\nAccording to Fiori3 design shortcuts should be on desktop devices."}}};const d=()=>j;d.parameters={docs:{story:{iframeHeight:400},description:{story:"\nMenu with an additional submenu that can be used for items that can be further grouped under a level but not necessarily visible to user always.\n\nFor a submenu, do the following:\n- Specify `fd-menu__link` class normally like other items. Use `has-child` class to apply styles for parent containing the submenu.\n- Create an addon indicating submenu level using `fd-menu__addon-after--submenu` class and an icon.\n- After the end of the `fd-menu__link` container, use `fd-menu__sublist` class in its own `<ul>`\n- Follow the same template for submenu as you would for a normal menu. The same `fd-menu__item` and `fd-menu__link` works for the subitems too.\n"}}};const t=()=>Q;t.storyName="Extended Menu List Items (Segmented Button)";t.parameters={docs:{story:{iframeHeight:400},description:{story:`Each view of the segmented button gets its own menu item. The selected Button in the Segmented Button gets the Active Dot in front of the Menu List Item and is always separated with a separator.
      The ‘segmented button menu items’ cannot have an icon in front. Instead, the place is reserved for the Active Dot.
`}}};const l=()=>Z;l.storyName="Extended Menu List Items (Toggle Button)";l.parameters={docs:{story:{iframeHeight:400},description:{story:`The toggle button gets “transformed” into a menu item which can be toggled and untoggled. If the toggle button is toggled, then a checkmark appears in front of the text (on position of an icon).
...
`}}};const i=()=>$;i.storyName="Extended Menu List Items (Input)";i.parameters={docs:{story:{iframeHeight:400},description:{story:`An Input field can be placed in a menu item.
